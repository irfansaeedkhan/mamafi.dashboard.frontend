...
import { InvoiceTableMobile } from '../dashboard/invoice-table-mobile';

export const RenderedComponent = () => {
  const params = useSearchParams();

  return params.get('tab') === 'terms' ? (
    <div className="--max-w-[1081px] mx-auto w-full p-4 sm:p-6 md:p-8">
      <h1 className="text-white py-6 text-3xl font-bold">Privacy Policy</h1>
      <p className={pClass}>
        <span>
      ...
      <h2 className="text-white py-8 text-xl font-semibold leading-5">
...
const pClass = 'flex flex-col gap-3 whitespace-pre-wrap break-words text-sm font-medium leading-6';

const boldClass = 'font-bold';
const subHeadingClass = ' text-base font-semibold';
